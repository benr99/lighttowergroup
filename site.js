(function () {
  ...
  function addSkipLink() {
    if (document.querySelector('.ltg-skip-link')) return;
    var target = document.querySelector('main, [role="main"]');
    if (!target) return;
    if (!target.id) target.id = 'main-content';
    var link = document.createElement('a');
    link.className = 'ltg-skip-link';
    link.href = '#' + target.id;
  ...
  function essayDeskToken() {
    ...
    if (params.get('essaydesk') === 'off') {
      sessionStorage.removeItem('ltgEssayDeskToken');
      return '';
    }
    var tokenFromUrl = params.get('essaydesk');
    if (tokenFromUrl) {
      sessionStorage.setItem('ltgEssayDeskToken', tokenFromUrl);
      params.delete('essaydesk');
      var clean = window.location.pathname + (params.toString() ? '?' + params.toString() : '') + window.location.hash;
      window.history.replaceState({}, document.title, clean);
      return tokenFromUrl;
    }
    return sessionStorage.getItem('ltgEssayDeskToken') || '';
  }
  ...
  function openEssayModal(packageData) {
    ...
    function addLabeledNote(container, label, value) {
      if (!value) return;
    ...
    var meta = modal.querySelector('#essay-modal-meta');
    meta.replaceChildren();
    [packageData.archetype || 'Essay', 'Score: ' + (score || 'n/a') + '/10', packageData.length_mode || 'standard']
    ...
    var notes = modal.querySelector('#essay-modal-notes');
    notes.replaceChildren();
    addLabeledNote(notes, packageData.fallback ? 'Review needed' : 'Package ready', packageData.fallback
      ? 'No generated Essay Desk package was found for this Insight yet.'
      : 'Generated by the Light Tower CRE Essay Desk.');
    addLabeledNote(notes, 'Hidden thesis', packageData.hidden_thesis);
    addLabeledNote(notes, 'Visual', visual.type ? visual.type + (visual.rationale ? ' — ' + visual.rationale : '') : '');
    addLabeledNote(notes, 'Follow-up', packageData.follow_up_comment);
    updateEssayCount();
